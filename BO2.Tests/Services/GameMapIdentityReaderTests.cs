...
namespace BO2.Tests.Services
{
    public sealed class GameMapIdentityReaderTests
    {
        ...
        [Fact]
        public void ReadMapIdentity_WhenBaseMapIsDieRise_ReturnsSupportedDieRiseWithoutStartLocation()
        {
            DetectedGame detectedGame = CreateSteamZombiesGame();
            FakeProcessMemoryAccessor memory = new();
            WriteDvarString(memory, "mapname", 0x02A32AA8U, 0x03000000U, " ZM_HIGHRISE ");
            var reader = new GameMapIdentityReader(memory);
            ...
            Assert.True(result.IsSupportedMap);
            Assert.NotNull(result.Identity);
            Assert.Equal("zm_highrise", result.Identity!.BaseMapToken);
            Assert.Null(result.Identity.StartLocationToken);
            Assert.Equal("zm_highrise", result.Identity.InternalMapToken);
            Assert.Equal("Die Rise", result.Identity.DisplayName);
        }

        [Fact]
        public void ReadMapIdentity_WhenBaseMapIsMobOfTheDead_ReturnsSupportedMobOfTheDeadWithoutStartLocation()
        {
            DetectedGame detectedGame = CreateSteamZombiesGame();
            FakeProcessMemoryAccessor memory = new();
            WriteDvarString(memory, "mapname", 0x02A32AA8U, 0x03000000U, " ZM_PRISON ");
            var reader = new GameMapIdentityReader(memory);
            ...
            Assert.True(result.IsSupportedMap);
            Assert.NotNull(result.Identity);
            Assert.Equal("zm_prison", result.Identity!.BaseMapToken);
            Assert.Null(result.Identity.StartLocationToken);
            Assert.Equal("zm_prison", result.Identity.InternalMapToken);
