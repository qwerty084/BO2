...
namespace BO2.Tests.Services
{
    public sealed class GameEventMonitorTests
    {
        ...
        {
            byte[] snapshot = CreateSnapshot(GameCompatibilityState.Compatible, droppedEventCount: 2, eventCount: 1);
        ...
        {
            byte[] snapshot = CreateSnapshot(GameCompatibilityState.Compatible, droppedEventCount: 0, eventCount: 0);
            ...
            string eventName)
        {
            ...
            nameBytes.AsSpan(0, Math.Min(nameBytes.Length, GameEventMonitor.MaxEventNameBytes))
                .CopyTo(snapshot.AsSpan(offset + 8, GameEventMonitor.MaxEventNameBytes));
        }
    }
}
